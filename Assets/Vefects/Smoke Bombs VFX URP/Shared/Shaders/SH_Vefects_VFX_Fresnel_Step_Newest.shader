	...
	SubShader
	{
		...
		Pass
		{
				 ...
				 ) : SV_Target
			{
				...
				float fresnelNode54 = ( input.ase_texcoord6.z + _Fresnel_Scale * pow( saturate( 1.0 - fresnelNdotV54 ), _Fresnel_Power ) );
				...
				float temp_output_95_0 = saturate( ( saturate( ( fresnelNode54 + saturate( ( 1.0 - distanceDepth47 ) ) ) ) * pow( saturate( tex2D( _Noise_01_Texture, ( Distortion37 + panner41 ) ).r * tex2D( _Noise_02_Texture, ( Distortion37 + panner42 ) ).r ) , _Opacity_Power ) ) );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_TARGET
			{
				...
				float3 ase_worldNormal = input.ase_texcoord4.xyz;
				float fresnelNdotV54 = dot( ase_worldNormal, ase_viewDirWS );
				float fresnelNode54 = ( input.ase_texcoord3.z + _Fresnel_Scale * pow( saturate( 1.0 - fresnelNdotV54 ), _Fresnel_Power ) );
				float4 ase_screenPosNorm = ScreenPos / ScreenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float screenDepth47 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth47 = saturate( ( screenDepth47 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input ) : SV_TARGET
			{
				...
				float fresnelNdotV54 = dot( ase_worldNormal, ase_viewDirWS );
				float fresnelNode54 = ( input.ase_texcoord.z + _Fresnel_Scale * pow( saturate( 1.0 - fresnelNdotV54 ), _Fresnel_Power ) );
		...
		Pass
		{
				 ...
				 )
			{
				...
				float fresnelNode54 = ( input.ase_texcoord2.z + _Fresnel_Scale * pow( saturate( 1.0 - fresnelNdotV54 ), _Fresnel_Power ) );
				float4 ase_screenPosNorm = ScreenPos / ScreenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float screenDepth47 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth47 = saturate( ( screenDepth47 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) );
