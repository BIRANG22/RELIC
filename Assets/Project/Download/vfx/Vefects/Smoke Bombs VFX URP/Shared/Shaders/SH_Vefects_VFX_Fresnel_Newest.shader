	...
	SubShader
	{
		...
		Pass
		{
				 ...
				 ) : SV_Target
			{
				...
				float clampResult96 = clamp( pow( max( ( tex2D( _Noise_Color_Texture, ( Distortion34 + panner106 ) ).r * _NoiseColor_Intensity ), 0.0001 ) , _NoiseColor_Power ) , 0.0 , 1.0 );
				...
				float fresnelNode75 = ( input.ase_texcoord4.z + _Fresnel_Scale * pow( max( 1.0 - fresnelNdotV75, 0.0001 ), _Fresnel_Power ) );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input ) : SV_TARGET
			{
				...
				float fresnelNdotV75 = dot( ase_worldNormal, ase_viewDirWS );
				float fresnelNode75 = ( input.ase_texcoord2.z + _Fresnel_Scale * pow( max( 1.0 - fresnelNdotV75, 0.0001 ), _Fresnel_Power ) );
		...
		Pass
		{
				 ...
				 )
			{
				...
				float fresnelNode75 = ( input.ase_texcoord3.z + _Fresnel_Scale * pow( max( 1.0 - fresnelNdotV75, 0.0001 ), _Fresnel_Power ) );
				float4 ase_screenPosNorm = ScreenPos / ScreenPos.w;
				ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
				float screenDepth67 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float distanceDepth67 = saturate( ( screenDepth67 - LinearEyeDepth( ase_screenPosNorm.z,_ZBufferParams ) ) / ( _DepthFade_Distance ) );
