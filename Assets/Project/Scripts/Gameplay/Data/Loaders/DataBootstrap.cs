...
namespace Relic.Gameplay.Data
{
    public class DataBootstrap
    {
        ...
        private SkillRangeIconDatabase skillRangeIconDatabase;
        ...
        public void SetSkillRangeIconDatabase(SkillRangeIconDatabase db)
        {
            skillRangeIconDatabase = db;
        }
        ...
        private void InjectSkillRangeIcons(List<SkillRangeData> ranges)
        {
            if (skillRangeIconDatabase == null)
            {
                Debug.LogWarning("[DataBootstrap] SkillRangeIconDatabase가 연결되지 않았습니다.");
                return;
            }

            skillRangeIconDatabase.Initialize();

            foreach (var range in ranges)
            {
                if (skillRangeIconDatabase.TryGetIcon(range.RangeId, out Sprite icon))
                    range.Icon = icon;
                else
                    Debug.LogWarning($"[DataBootstrap] SkillRangeIcon 없음: {range.RangeId}");
            }
        }
