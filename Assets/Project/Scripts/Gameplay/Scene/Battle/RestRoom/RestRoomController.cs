...
public class RestRoomController : MonoBehaviour
{
    ...
    [SerializeField] private string healCompleteMessage = "Î™®Îì† Ï∫êÎ¶≠?∞Ïùò Ï≤¥Î†•??30% ?åÎ≥µ?àÏäµ?àÎã§";
    ...
    public void OnUpgradeButtonClicked()
    {
        ...
        if (upgradePanel == null)
        {
            Debug.LogWarning("[RestRoomController] SkillUpgradePanel ?ÜÏùå");
            return;
        }
    ...
    private void RecoverAllPartyHPByRatio(float ratio)
    {
        ...
        Debug.Log($"[RestRoomController] Î™®Îì† ?åÌã∞??HP {Mathf.RoundToInt(safeRatio * 100f)}% ?åÎ≥µ ?ÑÎ£å");
    }
