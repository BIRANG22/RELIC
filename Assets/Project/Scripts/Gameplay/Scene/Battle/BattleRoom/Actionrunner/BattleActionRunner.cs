...
public class BattleActionRunner
{
    ...
    private IEnumerator RunSingleWithTimeout(ActionRoutine actionRoutine)
    {
        ...
        {
            Debug.LogError($"[BattleActionRunner] CoroutineHost ?ÜÏùå / Action:{actionRoutine.Label}");
            yield break;
        }
    ...
    private IEnumerator RunParallel(List<ActionRoutine> routines)
    {
        ...
        if (CoroutineHost.Instance == null)
        {
            Debug.LogError("[BattleActionRunner] CoroutineHost ?ÜÏùå");
            yield break;
        }
    ...
    private void TryPlaceMuckProjectileResidue(MonsterReservedCommand command)
    {
        ...
        // Î®∏ÌÅ¨???¨ÏÇ¨Ï≤??ºÌï¥?Ä ?îÏó¨Î¨??ºÌï¥??Î≥ÑÎèÑ???ÄÍ≤©ÏúºÎ°?Ï≤òÎ¶¨?©Îãà??
        // Î™©Ìëú Í∑∏Î¶¨?úÏóê Ï∫êÎ¶≠?∞Í? ?àÎã§Î©??îÏó¨Î¨??ùÏÑ± ÏßÅÌõÑ GR_Residue ?ºÌï¥Î•??∞Î°ú ?ÅÏö©?©Îãà??
        ...
        BattleUnitAnimator attackerAnimator)
    {
        ...
        {
            Debug.LogWarning($"[PlayerSkillEffect] EffectEntries ?ÜÏùå / Skill:{command.SkillData.SkillId}");
        ...
        BattleUnitAnimator attackerAnimator)
    {
        ...
        // ?§Îã® Í≥µÍ≤©?Ä ?ÄÍ≤??üÏàòÎßåÌÅº ?úÎ°ú ?§Î•∏ Í≥µÍ≤© Î™®ÏÖò??Îπ†Î•¥Í≤??¥Ïñ¥???¨ÏÉù?úÎã§.
        // 1??Í≥µÍ≤©Î≥¥Îã§ Í∞?Î™®ÏÖò???¨ÏÉù ?çÎèÑÎ•??íÏó¨ ?ÑÏ≤¥ ?âÎèô ?úÍ∞Ñ??Í≥ºÎèÑ?òÍ≤å Í∏∏Ïñ¥ÏßÄÏßÄ ?äÎèÑÎ°??úÎã§.
        ...
        string skillId)
    {
        ...
        {
            Debug.LogError(
                $"[PlayerSkillEffect] Effect ?§Ìñâ Ï§??êÎü¨ / " +
    ...
    private IEnumerator ExecuteMonsterCommand(MonsterReservedCommand command)
    {
        ...
        // ?¨ÌÉà ?¥Îèô?Ä ?§ÌÇ¨ ?∞Ïù¥?∞Ïùò ?Ä?ÑÎùº???úÍ∏∞?Ä Í¥ÄÍ≥ÑÏóÜ???¥Îèô Ï≤òÎ¶¨Î°?Î≥¥ÎÉÖ?àÎã§.
    ...
    private IEnumerator ExecuteMonsterMove(MonsterReservedCommand command)
    {
        ...
        // ?¨ÌÉà ?¥Îèô?Ä ?àÏïΩ???àÎ? Î™©Ï†ÅÏßÄÎ•??¨Ïö©?????àÏúºÎØÄÎ°??ºÎ∞ò ?¥Îèô??0 ?§ÌîÑ??Í≤Ä?¨Î≥¥??Î®ºÏ? Ï≤òÎ¶¨?©Îãà??
        if (IsNocturnPortalMove(command))
        {
        ...
        try
        {
            ...
            // Ï≤??¥Îèô Ïπ∏Ïù¥ ?¥Î? ?§Î•∏ ?†Îãõ?êÍ≤å ?êÏú†?òÏñ¥ ?àÎã§Î©???Ïπ∏ÎèÑ ?¥Îèô?òÏ? ?äÍ≥† Ï¶âÏãú Ï∂©Îèå?©Îãà??
            // ?¥Îèô?êÏ? ?¥Îãπ Ïπ∏ÏùÑ ?êÏú†???†Îãõ Î™®Îëê Ï∂©Îèå Í≥†Ï†ï ?ºÌï¥Î•?Î∞õÏäµ?àÎã§.
            ...
            // Î∏îÎ°≠?Ä ?¥Îèô???ÑÎ£å?????¥Îèô ??Í∑∏Î¶¨?úÏóê ?îÏó¨Î¨ºÏùÑ ?®ÍπÅ?àÎã§.
            // ?îÏó¨Î¨ºÏùò ?ºÌï¥???¥Îèô Í≥µÍ≤©Í≥?Î≥ÑÍ∞úÎ°?GridEffect ?∞Ïù¥?∞Ïóê ?∞Îùº ?ÅÏö©?©Îãà??
            if (monster.RuntimeData != null &&
        ...
        Vector2Int moveOffset)
    {
        ...
        // ?¨ÌÉà?Ä ?àÏïΩ???àÎ? Î™©Ï†ÅÏßÄÎ•??∞ÏÑ† ?¨Ïö©?©Îãà??
        // ?ûÏÑ† ?âÎèô?ºÎ°ú ?ÑÏû¨ ?ÑÏπòÍ∞Ä ?¨ÎùºÏßÄÎ©??àÏïΩ ?πÏãú???ÅÎ? ?§ÌîÑ?ãÏù¥ 0?????òÎèÑ ?àÏúºÎØÄÎ°?
        // ?àÎ? Î™©Ï†ÅÏßÄÍ∞Ä ?àÎäî Î™ÖÎ†π?Ä MoveOffset??0?¥Ïñ¥??Ï∑®ÏÜå?òÏ? ?äÏäµ?àÎã§.
        ...
        // ?àÏïΩ ?πÏãú???àÎ? Î™©Ï†ÅÏßÄÍ∞Ä ?àÏúºÎ©?Í∑∏Í≤É???∞ÏÑ† ?¨Ïö©?©Îãà??
        // ?ûÏÑ† ?¥Îèô?¥ÎÇò Í∑∏Îû© ?åÎ¨∏???πÌÑ¥???ÑÏû¨ ?ÑÏπòÍ∞Ä ?¨Îùº?∏ÎèÑ ?¨ÌÉà Î™©Ï†ÅÏßÄÍ∞Ä ?Ä?¥Ï?ÏßÄ ?äÏäµ?àÎã§.
        ...
        // ?§Ìñâ ÏßÅÏ†ÑÍπåÏ? Î™©Ï†ÅÏßÄ Ïπ∏Ïù¥ ÎπÑÏñ¥ ?àÏñ¥???©Îãà??
        // Ï∫êÎ¶≠?∞ÎÇò ?§Î•∏ Î™¨Ïä§?∞Í? ?¥Îãπ Ïπ∏ÏùÑ Ï∞®Ï??àÎã§Î©??¨ÌÉà ?¥Îèô??Ï∑®ÏÜå?©Îãà??
        ...
        // Î™©Ï†ÅÏßÄ Í≤Ä?¨Î? ?µÍ≥º?àÏúºÎØÄÎ°??¥Î≤à ?¨ÌÉà?Ä ?±Í≥µ ?ÅÌÉúÎ°?Í∏∞Î°ù?©Îãà??
        ClearNocturnPortalFailed(command);

        // ?¥Îèô ?∞Ï∂ú???úÏûë?òÎäî ?úÍ∞Ñ Î™©Ï†ÅÏßÄ ?àÍ≥† ?¥Î?ÏßÄÎ•??úÍ±∞?©Îãà??
        HideNocturnPortalDestinationIndicator(command);
        ...
        // ?¨ÌÉà ?¥Îèô???¥Îèô ?ÅÌÉú?ÑÏùÑ Î≥¥Ïó¨Ï£ºÍ∏∞ ?ÑÌï¥ Move ?†ÎãàÎ©îÏù¥?òÏùÑ ?¨ÏÉù?©Îãà??
        // ?ÑÏπò Î≥¥Í∞Ñ?Ä ?¨Ïö©?òÏ? ?äÍ≥† Î™©Ï†ÅÏßÄ?êÎäî Ï¶âÏãú ?òÌ??©Îãà??
        BattleUnitAnimator animator = monster.GetComponent<BattleUnitAnimator>();
        ...
        // ?ºÎ¶¨ Í∑∏Î¶¨?úÏ? ?îÎ©¥ ?ÑÏπòÎ•??®Íªò Í∞±Ïã†???§Ï†ú ?êÏú† ?ÑÏπò???ÑÎ∞© Ïπ∏ÏúºÎ°??¥Îèô?úÌÇµ?àÎã§.
        ...
        Vector2Int requestedOffset)
    {
        ...
        for (int i = 0; i < monster.OccupiedGridIndices.Count; i++)
        {
            ...
            // Îß??∏Í≥Ω?Ä Ï∂©Îèå ?ºÌï¥ ?Ä?ÅÏù¥ ?ÑÎãô?àÎã§.
            if (!gridManager.IsValidCoord(nextCoord))
                return false;
    ...
    private IEnumerator ExecuteMonsterSkill(MonsterReservedCommand command)
    {
        ...
        // ÏßÅÏ†Ñ ?¨ÌÉà???êÏú† ?êÎäî ?¥Îèô Î∂àÍ?Î°?Ï∑®ÏÜå?êÎã§Î©??àÏïΩ???¨ÌÉà ?ÑÏπòÎ•?Í≥µÍ≤© ?êÏ†ê?ºÎ°ú ?¨Ïö©?òÏ? ?äÏäµ?àÎã§.
        // ?πÌÑ¥???§Ï†ú ?ÑÏû¨ ?ÑÏπòÎ•??êÏ†ê?ºÎ°ú Î∞îÍæ∏Í≥? ?ÑÏû¨ ?ÑÏπò?êÏÑú ?§Ï†ú ?Ä?ÅÏùÑ ?•Ìï¥ Î∞©Ìñ•???§Ïãú ?ïÌï©?àÎã§.
        ...
        // AIÍ∞Ä Í≥µÍ≤© Î∞©Ìñ•??ÏßÄ?ïÌïú Í≤ΩÏö∞ ?àÏïΩ??Î∞©Ìñ•???§Ìñâ ?úÏ†ê?êÎèÑ Í∑∏Î?Î°??¨Ïö©?©Îãà??
        // Î≤îÏúÑ???§Î•∏Ï™ΩÏù∏???†ÎãàÎ©îÏù¥?òÎßå ?ºÏ™Ω???•Ìïò???ÑÏÉÅ??Î∞©Ï??©Îãà??
        ...
        // ?ïÏ†ï??Î∞©Ìñ•??Í∏∞Ï??ºÎ°ú Í≥µÍ≤© Î≤îÏúÑÎ•?Í≥ÑÏÇ∞?©Îãà??
        ...
        // Í∞ïÏ†ú Î∞©Ìñ•???ÜÎäî ?ºÎ∞ò AI ?âÎèôÎß??§Ï†ú Î™ÖÏ§ë ?Ä??Ï™ΩÏúºÎ°??åÏ†Ñ?©Îãà??
        // ?¨ÌÉà ?ÑÏÜç Í≥µÍ≤©Ï≤òÎüº Î∞©Ìñ•???àÏïΩ???âÎèô?Ä ?ÑÏóê??ÏßÄ?ïÌïú Î∞©Ìñ•???†Ï??©Îãà??
        ...
        try
        {
            ...
            // Î®∏ÌÅ¨???¨ÏÇ¨Ï≤¥Îäî Î™ÖÏ§ë ?¨Î??Ä Í¥ÄÍ≥ÑÏóÜ???àÏïΩ??Î™©Ìëú Í∑∏Î¶¨?úÏóê ?îÏó¨Î¨ºÏùÑ ?ùÏÑ±?©Îãà??
            // ?¨ÏÇ¨Ï≤??ºÌï¥?Ä ?îÏó¨Î¨??ºÌï¥??Î∂ÑÎ¶¨?òÎ©∞, ?ùÏÑ± ?úÍ∞Ñ?êÎäî ?îÏó¨Î¨??ºÌï¥Î•?Ï¶âÏãú ?ÅÏö©?òÏ? ?äÏäµ?àÎã§.
            ...
            // ?†Îçî???êÌè≠?Ä Í≥µÍ≤© ?®Í≥º ?ÅÏö©???ùÎÇú ???êÏã†???úÍ±∞?©Îãà??
            // ?ºÎ∞ò Ï≤òÏπòÍ∞Ä ?ÑÎãàÎØÄÎ°??òÎÑå?? Í≥†Ïú† ?ÑÏù¥?? ?†Î¨º ?±Ïùò ?¨Îßù Î≥¥ÏÉÅ?Ä ?òÏßë?òÏ? ?äÏäµ?àÎã§.
            if (monster.RuntimeData != null &&
        ...
        BattleUnitAnimator monsterAnimator)
    {
        ...
        for (int hitIndex = 0; hitIndex < hitCount; hitIndex++)
        {
            ...
            // Î™ÖÏ§ë ?Ä?ÅÏù¥ ?ÜÏñ¥??Î™¨Ïä§?∞Ïùò Í≥µÍ≤© Î™®ÏÖò?Ä Î∞òÎìú???¨ÏÉù?©Îãà??
            // ?Ä??Î∂Ä?¨Îäî ?ºÌï¥ ?ÅÏö©Îß?Í±¥ÎÑà?∞Î©∞, Í≥µÍ≤© ?úÎèÑ ?êÏ≤¥Î•?Ï∑®ÏÜå?òÏ? ?äÏäµ?àÎã§.
            if (monsterAnimator != null)
                monsterAnimator.PlayMonsterSkillAction(command);
    ...
    private IEnumerator ExecuteMonsterDashAttack(MonsterReservedCommand command)
    {
        ...
        for (int step = 1; step <= maxMove; step++)
        {
            ...
            {
                // Í∑∏Î¶¨???∏Í≥Ω?Ä Ï∂©Îèå ?Ä?ÅÏù¥ ?ÑÎãàÎØÄÎ°? Îß?Î∞ñÏúºÎ°??òÍ?ÏßÄ ?äÍ≥† Î©àÏ∂îÍ∏∞Îßå ?©Îãà??
                break;
            }

            if (!CanMonsterDashToOffset(monster, testOffset, out BattleCharacter blockingPlayer))
            {
                // ?•Ïï†Î¨ºÏù¥???§Î•∏ ?†Îãõ??ÎßâÌûå Í≤ΩÏö∞?êÎßå Ï∂©ÎèåÎ°?Ï≤òÎ¶¨?©Îãà??
