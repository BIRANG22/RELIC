...
public sealed class LobbySkillUpgradePanelUI : MonoBehaviour
{
    ...
    {
        Debug.Log($"[LobbySkillUpgradePanelUI] Í∞ïÌôî Î≤ÑÌäº ?ÖÎ†•. ?†ÌÉù ?¨Î?: {selection.HasSelection}");
        ...
        {
            Debug.LogError("[LobbySkillUpgradePanelUI] DataManager.InstanceÍ∞Ä ?ÜÏñ¥ Í∞ïÌôîÎ•??§Ìñâ?????ÜÏäµ?àÎã§.");
            return;
        }
        ...
        if (!result.Succeeded)
        {
            Debug.LogWarning($"[LobbySkillUpgradePanelUI] Í∞ïÌôî ?§Ìå®: {result.Failure}, Í∞ÄÍ≤? {result.Price}");
            ShowFailure(result.Failure);
            RefreshPrice();
            return;
        }

        Debug.Log($"[LobbySkillUpgradePanelUI] Í∞ïÌôî ?±Í≥µ. ?åÎ™®: {result.Price}, ?îÏï°: {lobby.BlueDustium}");
    ...
    private void OnItemClicked(SkillUpgradeRequest request, Sprite icon)
    {
        ...
        Debug.Log($"[LobbySkillUpgradePanelUI] ?§ÌÇ¨ ?¥Î¶≠ ?†ÌÉù: {request.CurrentSkillId} -> {request.UpgradeSkillId}");
    ...
    {
        // ???®ÎÑê?Ä ?∏Î≤Ñ ???ÅÏÑ∏ ?ïÎ≥¥Í∞Ä ?úÏãú?òÎ?Î°? ?îÎ©¥??ÎØ∏Î¶¨Î≥¥Í∏∞ ?Ä?ÅÍ≥º
        // ?§Ï†ú Í∞ïÌôî ?Ä?ÅÏùÑ ?ôÏùº?òÍ≤å ?†Ï??úÎã§.
        selection.Select(request);
        ...
        SetText(nameText, string.IsNullOrWhiteSpace(current.Name) ? current.SkillId : current.Name);
        SetText(upgradedNameText, string.IsNullOrWhiteSpace(upgraded.Name) ? upgraded.SkillId : upgraded.Name);
