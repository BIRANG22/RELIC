...
- Impact-only projectile target-grid VFX uses a runtime `BattleVfxEntry` with `BattleVfxRenderMode.DirectWorldRenderer`. Arabella's lightning impact prefab can lose the vertical strike portion when several copies are captured through separate board-proxy RenderTextures, while its shockwave remains visible. Direct world rendering keeps every lightning instance visible.
- The direct impact path is still spawned from a detached anchor at the resolved `GridCell` position, not from the caster transform, so it does not reintroduce the old mirrored board-quadrant placement.
